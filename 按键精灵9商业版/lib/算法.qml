...
Function 提取字串符中所有数字(字符串)
    ...
    TQstring = TQstring & "rExp.Pattern = ""\d+""" & vbCrLf    //设置过滤模式
    TQstring = TQstring & "rExp.Global = True" & vbCrLf        //设置全局可用性
    TQstring = TQstring & "Dim Num, Nums" & vbCrLf             //定义变量
    TQstring = TQstring & "Set Nums = rExp.Execute(""" & 字符串 & """)" & vbCrLf   //执行搜索  
    TQstring = TQstring & "For Each Num In Nums" & vbCrLf      //遍历匹配集合
...
Function 得到字符串中字母的数量(字符串)
    ...
    Dim TQstring  
    TQstring = "Dim regEx, Matches, shuliang" & vbCrLf                    //定义变量
    TQstring = TQstring & "Set regEx = New RegExp" & vbCrLf               //建立正则表达式
    TQstring = TQstring & "regEx.Pattern = ""([a-z]{1})""" & vbCrLf       //设置过滤模式
    TQstring = TQstring & "regEx.IgnoreCase = true" & vbCrLf              //设置是否区分字符大小写
    TQstring = TQstring & "regEx.Global = True" & vbCrLf                  //设置全局可用性
    TQstring = TQstring & "Set Matches = regEx.Execute(""" & 字符串 & """)" & vbCrLf   //执行搜索   
